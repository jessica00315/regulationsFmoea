...
st.set_page_config(page_title="法規 MHTML 轉 HTML 工具", layout="wide")

st.title("📘 法規 MHTML 上傳與格式轉換工具")
st.markdown("請上傳從 [經濟部法規網站](https://law.moea.gov.tw) 下載的 `.mhtml` 檔案，系統會解析並生成可互動的 `.html` 條文檔案。")
...
if content_div:
    ...
    articles = re.split(r'(?=\n?\s*[一二三四五六七八九十]{1,3}、)', raw_text)
    for idx, art in enumerate(articles):
        match = re.match(r'\s*([一二三四五六七八九十]{1,3})、', art)
        art_no = match.group(1) if match else f"{idx+1}"
        content = art.strip()
        law_data.append({
            '章': '',
            '章節': '',
...
<table><thead><tr><th>章</th><th>章節</th><th>條</th><th width="500wh">條文內容</th><th>定義條文</th><th>是否適用</th><th>是否符合</th><th width="300wh">說明</th></tr></thead><tbody>
'''

for row in law_data:
    content = html.escape(row['條文內容']).replace('\n', '<br>')
    html_string += f'''<tr>
    <td>{row['章']}</td><td>{row['章節']}</td><td>{row['條']}</td><td>{content}</td>
